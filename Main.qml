...
Item {
    ...
    Item {
        ...
        Image {
            // Background
            id: backgroundImage
            // `background` puede ser un archivo o varias imágenes separadas por "|":
            // con varias, cambian solas cada Config.slideshowInterval segundos.
            property string fondos: root.state === "lockState" ? Config.lockScreenBackground : Config.loginScreenBackground
            property var lista: fondos.split("|").filter(function (n) { return n.length > 0; })
            property int indice: 0
            property string tsource: lista.length > 0 ? lista[indice % lista.length] : ""
            onListaChanged: indice = 0
            ...
            Timer {
                interval: Math.max(5, Config.slideshowInterval) * 1000
                repeat: true
                running: backgroundImage.lista.length > 1 && !backgroundImage.isVideo
                onTriggered: backgroundImage.indice = (backgroundImage.indice + 1) % backgroundImage.lista.length
            }
